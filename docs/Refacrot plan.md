...
ScoredTask extends WalkerTask with score: number + scoreBreakdown: { dealImportance: number, timePressure: number, immovability: number }
...
Score = dealImportance + timePressure + immovability + graphPressure + anomalyBoost
...
function computeScore(task: ScoredTask, deal: Deal, settings: UserSettings): number {
  ...
  const dealImportance = nVal * roleMultiplier * stageMultiplier
  ...
  // Immovability + anomaly
  const immovability = task.weight
  const anomaly = deal.anomaly_boost

  return Math.round(dealImportance + timePressure + graphPressure + immovability + anomaly)
}

The formula is still additive (no term can zero out another). The new terms are all additive bonuses from graph structure — they don't break existing behavior, they refine it.
...
Scoring engine: Expanded formula with graphPressure term (blocking fanout, critical path, CP responsiveness, stalled deal penalty). All additive, all deterministic.
...
Temporal DSL: TypeScript, ~20-30 functions, LLM generates calls, execute in sandbox.
