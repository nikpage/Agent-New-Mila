...
Step 5: Generate action proposals via 3-call triage pipeline: extractMessageFacts (flash-lite, temp 0 — pure reading comprehension, extracts what CP said/asked/deadlines/venue/time/questions) → triageConversation (flash with thinking — decision-only, receives verified facts not raw email, decides needs_action/type/urgency) → verifyTriage (flash-lite, cross-checks justification against source message). Extraction fields (venue, time, questions, what_cp_wants) are code-merged onto triage result in planning.ts, not AI-generated by the decision model. If triage returns revisit_at, conversation is snoozed until that date. Channel-aware, batched ×5.
...
### Triage Architecture: Extract → Decide → Verify

The triage pipeline separates reading comprehension from decision-making. This is an architectural choice, not a prompt optimization — do not collapse these back into one call.

**Why it's split**: A single AI call that simultaneously extracts facts, decides action type, picks urgency, selects venue/time, generates intent text, and collects questions will hallucinate when confused on any one task. Splitting means: if extraction is wrong, fix the extraction prompt. If decisions are wrong, fix the decision prompt. Each step is independently testable.

**Pipeline** (orchestrated in `planning.ts`):
```
extractMessageFacts()     → ExtractionResult (verified facts: what CP said, deadlines, venue, time, questions)
       ↓
triageConversation()      → TriageResult (decision: needs_action, type, urgency, intent_cs, rationale_cs)
       ↓
code merge in planning.ts → copy extraction fields (venue, time, questions, what_cp_wants) onto TriageResult
       ↓
verifyTriage()            → binary check: is this action justified or just FYI?
```

**Key design rules**:
- `extractMessageFacts()` sees the raw email. `triageConversation()` does NOT — it sees only the ExtractionResult.
- Venue, time, questions_for_user, and what_cp_wants are **code-copied** from extraction to triage result. The decision model cannot override them.
- `verifyTriage()` still sees the raw email for independent cross-checking.
- The `TriageResult` interface is unchanged — downstream code (scheduling, brief rendering, action creation) is unaffected.

**ExtractionResult fields**: `what_cp_said`, `what_cp_asks_for[]`, `deadlines[]` (text + resolved_date + consequence + is_hard), `confirmed_venue_index`, `confirmed_venue_freetext`, `confirmed_time_index`, `confirmed_time_freetext`, `questions_for_user[]`, `cp_commitments[]`.

**DO NOT** re-merge extract and decide into a single call. The split exists because the monolithic call was the root cause of 6 persistent bugs (hallucinated questions, missed deadlines, wrong venues, misread emails). Prompt tweaks did not fix them — only the architectural separation did.
