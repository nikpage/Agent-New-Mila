...
The CP's explicit request IS the primary action. Do not bury it inside preparatory tasks. If CP says "potvrďte do 17:00" (confirm by 5pm), the FIRST item in any TODO list must be "Potvrďte obchod do 17:00" — document prep comes AFTER. Each deadline in the conversation maps to its own urgency: "do 17:00 dnes" = urgency 10 (TODAY), "zítra v 9:00" = urgency 9 (TOMORROW). When the same conversation has BOTH a today deadline AND a tomorrow meeting, the urgency is the EARLIER one (10, not 9).
...
3. TODO — something the user needs to do themselves that is NOT a message and NOT a meeting. Examples: gather documents, review a contract internally, get banker approval, verify an address, prepare specific paperwork. NEVER use TODO when the CP proposed a meeting — that is SCHEDULE. NEVER use TODO when the next step is responding to the CP — that is REPLY or SCHEDULE.
   ...
   GOOD: "1. Potvrďte obchod Novotného do 17:00 (email nebo telefon)\n2. Zajistěte list vlastnictví (ne starší 3 dny)\n3. Vyžádejte bezdlužnost SVJ od správce"
