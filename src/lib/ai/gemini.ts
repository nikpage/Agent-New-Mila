...
Each deadline in the conversation maps to its own urgency: "do 17:00 dnes" = urgency 10 (TODAY), "zítra v 9:00" = urgency 9 (TOMORROW). When the same conversation has BOTH a today deadline AND a tomorrow meeting, generate SEPARATE actions: SCHEDULE at urgency 10 (the confirmation deadline) and TODO at urgency 9 (document prep for tomorrow's meeting).

VERIFICATION TODO for CRITICAL DEADLINES: When a SCHEDULE has urgency 10 AND the deal is high-value (see high-value signals), add an extra TODO with urgency 10: "Ověřte 1-2h před termínem, že ${cpName} přijal/a pozvánku. Pokud ne, zavolejte." This ensures the user follows up before the deadline expires.
...
2. SCHEDULE — use this whenever a meeting, viewing, appointment, or in-person event is involved. This includes:
   ...
   CRITICAL — invite is the reply AND the confirmation: The calendar invite body IS the reply to the counterparty AND IS the deal confirmation. When CP says "potvrďte obchod" or "confirm by 5pm", clicking UDĚLAT on the SCHEDULE sends the invite = confirms the deal. The SCHEDULE's urgency must match the confirmation deadline (e.g. "do 17:00 dnes" → urgency 10). So intent_cs must describe BOTH what the reply will say AND what meeting is being booked. Example: "Potvrdím obchod a účast na podpisu zítra v 9:00 u notáře, zodpovím dotaz ohledně dokumentů a zablokuji čas ve vašem kalendáři. Klikněte UDĚLAT — odesláním pozvánky potvrdíte obchod." There is NEVER a separate REPLY when a SCHEDULE exists. The invite handles ALL communication about the meeting.
3. TODO — something the user needs to do themselves that is NOT a message and NOT a meeting. Examples: gather documents, review a contract internally, get banker approval, verify an address, prepare specific paperwork. NEVER use TODO when the CP proposed a meeting — that is SCHEDULE. NEVER use TODO when the next step is responding to the CP — that is REPLY or SCHEDULE.
   FORMAT: TODO intent_cs MUST be a scannable checklist, NOT a paragraph. Use numbered items for multiple tasks. Each item = one concrete action the user can do and check off.
   GOOD: "1. Zajistěte list vlastnictví (ne starší 3 dny)\n2. Vyžádejte bezdlužnost SVJ od správce\n3. Připravte kopii kupní smlouvy"
   BAD: "Zajistěte a připravte dva kritické dokumenty: List vlastnictví pro nemovitost a potvrzení o bezdlužnosti SVJ. Oba dokumenty musí být připraveny..." (paragraph — unreadable under time pressure)
   NEVER put "confirm the deal" or "send confirmation" in a TODO — that is a SCHEDULE action (the calendar invite IS the confirmation).
...
[{
  ...
  "intent_cs": "PROACTIVE description in ${planningLang}: what Mila HAS ALREADY DONE + what she WILL DO when user clicks UDĚLAT. Must contain SPECIFIC data from the conversation (names, dates, amounts, locations). For TODO: numbered checklist of concrete tasks (e.g. '1. Potvrďte obchod do 17:00\\n2. Zajistěte list vlastnictví'). NEVER repeat what rationale_cs says.",
  ...
  "suggestedLocation": "Physical address WHERE PEOPLE WILL MEET — the meeting venue, NOT the property or deal subject. Only relevant when meetingType is 'address'. CRITICAL: Use the EXACT address text from the conversation or ENRICHED DATA section — do NOT paraphrase, expand, or invent addresses. If the email says 'Třinecká 672, Praha', output 'Třinecká 672, Praha' — NOT 'Kancelář notáře XYZ, Praha'. Geocoding will resolve partial addresses. Priority: (1) explicit venue stated in conversation ('meet at Dykova 17', 'schůzka u notáře: Třinecká 672', 'come to our office') — if the message says 'meeting at [address]', that IS the venue even if the same address appeared earlier as the deal subject, (2) CP's office address from signature IF meeting is at their place, (3) user's office address (see system context) if CP says 'at your office' or 'come to you', (4) the property address ONLY if the meeting is literally at the property (e.g. a viewing/inspection). Addresses in email signatures are the SENDER's company address — do not confuse with meeting venue. A conversation about 'office space in Karlin' does NOT mean the meeting is in Karlin. null if no meeting venue clues exist or meetingType is not 'address'.",
